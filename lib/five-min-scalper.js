...
// ─── Config ──────────────────────────────────────
const TRADE_SIZE = 1.00;          // Polymarket minimum
...
export class FiveMinScalper {
  ...
  async _tickIdle() {
    ...
    // Only care about the last 60s of the window
    if (secsLeft > WATCH_START || secsLeft <= ENTRY_TRIGGER) return;
    ...
    // Check bankroll (only when timing is right — avoids API spam)
    const bal = await this.clobClient.getBalance();
    if (bal < TRADE_SIZE) return; // Not enough for a trade

    // Enter watching phase
    this.state = WATCHING;

...
const MAX_POSITIONS = 2;             // 1d + 5m simultaneously
...
const ALLOWED_TF = new Set(["5m", "1h", "4h", "1d"]);  // 5m re-added for market fetching (separate FiveMinScalper handles entries)
...
export class Scalper {
  ...
  async reconcileOnChain() {
      ...
      try {
        ...
        if (onChainShares >= 0.5) {
          if (Math.abs(onChainShares - p.size) > 0.1) {
            ...
            // Recalc costBasis from actual shares × entry price
            p.costBasis = onChainShares * (p.entryPrice || 0.50);
          }
          reconciled.push(p);
        } else {
    ...
    // ALWAYS sync bankroll to actual wallet balance (fixes drift between tracked vs real)
    const walletBal = await this._orderClient.getBalance();
    if (walletBal > 0) {
      const lockedInPositions = this.positions.reduce((s, p) => s + (p.size * (p.entryPrice || 0.50)), 0);
      this.bankroll = walletBal;
      this.startingBankroll = walletBal + lockedInPositions;
      this.log("RECONCILE", `Bankroll synced: $${walletBal.toFixed(2)} available + $${lockedInPositions.toFixed(2)} locked`);
    }
    await this.save();
  }
